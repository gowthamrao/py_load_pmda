...
initial_data = {
    "jader_demo": pd.DataFrame({
        "identification_number": [1], "gender": ["Male"]
    }),
...
updated_data = {
    "jader_demo": pd.DataFrame({
        "identification_number": [1, 2], "gender": ["Male", "Female"] # Update 1, New 2
    }),
    "jader_drug": pd.DataFrame({
        "drug_id": ["drug1", "drug2"], "identification_number": [1, 2], "drug_name": ["Aspirin Forte", "Ibuprofen"] # Update drug1, New drug2
    }),
    "jader_reac": pd.DataFrame({
        "reac_id": ["reac1", "reac2"], "identification_number": [1, 2], "adverse_event_name": ["Severe Headache", "Nausea"] # Update reac1, New reac2
    }),
    "jader_hist": pd.DataFrame({
        "hist_id": ["hist1", "hist2"], "identification_number": [1, 2], "past_medical_history": ["Hypertension", "Asthma"] # Update hist1, New hist2
    }),
}
...
def test_jader_pipeline_merge_logic(mock_db_adapter, mock_etl_components, mocker):
    ...
    called_tables = [call.kwargs['target_table'] for call in mock_db_adapter.execute_merge.call_args_list]
    ...
    assert mock_db_adapter.execute_merge.call_count == 4
    called_tables_run2 = [call.kwargs['target_table'] for call in mock_db_adapter.execute_merge.call_args_list]
